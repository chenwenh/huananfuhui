<template>
    <div>
        ...
        <el-button type="primary" @click="addProject" size="medium" style="margin-left:20px;float:right;" class="radiusNone">添加</el-button>
        <!-- 表格 -->
        <Table
              ref="tableRef"
              :mainTable="mainTable"
        ...
        <dialogCommonComponent ref="dialogCommonComponent" :title="title" width="50%">
         ...
         <!-- 详情 -->
        <dialogCommonComponent ref="dialogCommonComponent2"  width="50%">
            <detail ref="detail" @search="search"></detail>
        </dialogCommonComponent>
    </div>
</template>
  ...
  methods: {
    ...
    deleteProject(row){
      ...
      this.$confirm('请确认要删除吗？', '提示', {
          ...
          vm.$http.delete(`${vm.$apiUrl.notice.delete}/${row.id}`)
          .then(res=>{
